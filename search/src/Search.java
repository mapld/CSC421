...
public class Search {
    ...
    int limit = 0;
    while(limit++ >= 0){
        String answer = TreeSearchDepthLimited(new FrontierFIFO(), limit);
        if(answer != null){
            return answer;
        }
    }
	...
	public String IterativeDeepeningGraphSearch() {
      ...
      int limit = 0;
      while(limit++ >= 0){
          String answer = GraphSearchDepthLimited(new FrontierFIFO(), limit);
          if(answer != null){
              return answer;
          }
      }
...
//For statistics purposes
  ...
  public void PrintTree(Node node, Map<Node,ArrayList<Node>> nodeChildren){
      char[] spaces = new char[node.depth];
      Arrays.fill(spaces, ' ');
      double g = node.path_cost;
      double h = problem.h(node.state);
      String order_str = "";
      if(node.order >= 0){
          order_str = "order=" + node.order;
      }

      System.out.println(new String(spaces) + node.state + "(g=)" + g + ", h=" + h + ", f=" + (g+h) + ")" + order_str);
      List<Node> children = nodeChildren.get(node);
      if(children == null){
          return;
      }
      for(int i = 0; i < children.size(); i++){
          PrintTree(children.get(i), nodeChildren);
      }
  }

  public void PrintTree(){
      Map nodeChildren = new HashMap<Node,ArrayList<Node>>();
      for(int i = 0; i < node_list.size();i++){
          Node cur = node_list.get(i);
          if(cur != initialNode){
              List<Node> childList;
              if(nodeChildren.containsKey(cur.parent_node)){
                  childList = (List)nodeChildren.get(cur.parent_node);
              }
              else{
                  childList = new ArrayList<Node>();
                  nodeChildren.put(cur.parent_node, childList);
              }
              childList.add(cur);
          }
      }
      PrintTree(initialNode, nodeChildren);
  }
