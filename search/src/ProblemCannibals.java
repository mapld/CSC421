...
public class ProblemCannibals extends Problem {
    ...
    private boolean isValid(StateCannibals state)
    {
        ...
        if(state.canArray[cannR] + state.canArray[cannL] > 3) return false;
        if(state.canArray[missR] + state.canArray[missL] > 3) return false;
        if(state.canArray[boatR] + state.canArray[boatL] > 1) return false;

        //Now, checking if cannibals out number missionaries
        if(state.canArray[boatL] == 1 && state.canArray[cannR] > state.canArray[missR]) return false;
        if(state.canArray[boatR] == 1 && state.canArray[cannL] > state.canArray[missL]) return false;
